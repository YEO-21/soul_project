...
public abstract class PlayerAttackBase
{
    ...
    public virtual PlayerAttackBase Initialize(PlayerCharacter owner, in PlayerAttackInfo attackInfo)
    {
        ...
        if (!this.attackInfo.m_AllowDuplicateDamage)
        {
    ...
    /// 연계 가능한 공격 코드로 변환합니다.
    /// </summary>
    /// <param name="attackCode">이 공격 객체와 비교될 공격 코드가 전달됩니다.</param>
    ...
    {
        // 중복 공격을 허용하지 않는 경우
        if (!attackInfo.m_AllowDuplicateDamage)
        {
